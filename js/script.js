window.addEventListener('DOMContentLoaded', (e) => {
    ...
    let boton = document.getElementById("btn-subscribir");
    boton.addEventListener("click", (ev) => {
        try {
            ...
            let subscriptor = { // JSON =  JavaScript object notation 
                // clave: valor
            ...
            console.dir(subscriptor);
            guardarsubscriptor( subscriptor );
...
async function guardarSubscriptor( subscriptor ) {
    const url = "https://diplomado-frontweb-default-rtdb.firebaseio.com/subscriptores.json";
    // alternativas:
    // 1) Callbacks     
    /*
    fetch(url, {
        method: "POST",
        body:   JSON.stringify(subscriptor) 
    })
        .then( respuesta => respuesta.json() ) // devuelve una Promesa 
        .then( data => mostrarExito("Se guardó correctamente su subscripción") )
    ;
    */
